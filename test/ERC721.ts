...
describe('ERC721 Test Suite', function () {
  ...
  before(async function () {
    ...
    erc721 = await factory.deploy("TokenName", 'NFTSym', {gasLimit: 1000000});
    await erc721.mint(signers[0].address, tokenId, {gasLimit: 1000000});
  })

  it('should be able to execute approve(address,uint256)', async function () {
    const res = await erc721.approve(signers[1].address, tokenId, {gasLimit: 1000000});
    console.log(`Approval Hash: ${res.hash}`);
  });

  it('should be able to execute transferFrom(address,address,uint256)', async function () {
    const res = await erc721.transferFrom(signers[0].address, signers[1].address, tokenId, {gasLimit: 1000000});
